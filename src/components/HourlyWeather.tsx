...
const HourlyWeather = ({ weatherInfo }: any) => {
  const settings = {
    dots: false,
    infinite: false,
    speed: 1000,
  ...
  const getHour = (hour: string | number | Date) => new Date(hour).getHours();

  const getHourly = (item: any, i: React.Key) => {
  ...
  return (
    <div className="hourly_screen">
      ...
      <Slider {...settings}>
        {weatherInfo.hourly.slice(0, 24).map((item: any, i: number) => {
